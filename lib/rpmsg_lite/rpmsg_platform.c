...
#endif
static const struct device *ipm_handle = ((void *)0);

// void platform_ipm_callback(void *context, uint32_t id, volatile void *data)
void platform_ipm_callback(const struct device *ipmdev, void *user_data,
			       uint32_t id, volatile void *data)
{
/*    printk("platform_ipm_callback: name %s id=%d\n", ipmdev->name, id);
    printk("platform_ipm_callback: data=%d\n", *(uint32_t *)data); */
    
    if (id != RPMSG_MU_CHANNEL)
    {
        printk("platform_ipm_callback: id != RPMSG_MU_CHANNEL\n");
        return;
    }
...
int32_t platform_init(void)
{
    ...
    // ipm_handle = DEVICE_DT_GET(DT_NODELABEL(mailbox0)); // DT_NXP_IMX_MU_MU_B_LABEL
    
    ipm_handle = device_get_binding(DT_N_S_soc_S_mailbox_30ab0000_FULL_NAME);
    if (!ipm_handle)
    {
        printk("platform_init: device_get_binding failed for %s\n", DT_N_S_soc_S_mailbox_30ab0000_FULL_NAME);
        return -1;
    }
